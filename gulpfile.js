...
function mini(cb) {
    ...
    src("src/root/*.png")
        ...
        src("src/root/index.html", { allowEmpty: true })
            ...
            .pipe(replace("{DESCRIPTION}", package.description, replaceOptions))
            .pipe(replace("{COPYRIGHT}", package.author.name, replaceOptions))
            .pipe(replace("{KEYWORDS}", package.keywords, replaceOptions))
            .pipe(replace("{AUTHOR}", package.author.name, replaceOptions))
