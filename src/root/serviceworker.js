/* To force update on all clients you have to change the version number.
   ...
   Just type a random string here to ensure update: qweqweqwe
*/

var version = "{ID_NAME}_{VERSION}";
var name = "[Worker] " + version + ": ";
var debug;
var logfetches;
...
self.addEventListener("message", function (event) {
	console.log(name + "%cMessage:" + event.data, "color:#cc9933");
})
		...
		*/
		caches.open(version).then(function (cache) {
			...
			*/
			return cache.addAll(
				[
					// List of resources to cache, example:
					//"game/assets/cooper_black.woff2"
				]
			);
		}).then(function () {
...
self.addEventListener("fetch", function (event) {
	if (event.request.cache === 'only-if-cached' && event.request.mode !== 'same-origin') {
		if (debug && logfetches) console.log(name + "%cFetch (prevented) " + event.request.url, "color:grey");
		return;
	}
	...
	event.respondWith(
		caches.match(event.request).then(function (cached) {
			var report;
			if (logfetches) {
				if (cached) console.log(name + "%cFetch (cached) " + event.request.url, "color:grey");
				else report = [name + "%cFetch (request) " + event.request.url, "color:grey"];
			}
			return cached || fetch(event.request).then(function (response) {
				var cacheCopy = response.clone();
				caches.open(version).then(function (cache) {
					if (debug) console.log(name + "%cFetch " + event.request.url, "color:#3333cc");
					report = null;
